...
$getProvId = $_GET['prov_id'];

if (isset($_GET['prov_id'])) {
	$query5 = "SELECT prov_name FROM tbl_province WHERE prov_id = '$getProvId'";

	$getProvName = mysqli_query($conn, $query5);
	checkQueryError($getProvName);

	$row5 = mysqli_fetch_assoc($getProvName);

	$provinceName = $row5['prov_name'];
}


?>

<title>Explore Products by Provinces</title>
...
<div class="container main-page search-product">
	...
	<div class="row">
		...
		<div class="col-sm-9">
			<h1><?php echo $provinceName; ?></h1>

<!-- 			<div class="row">
				<div class="col-sm-3 offset-9">
					<select name="sort" id="sort" class="form-control"  onchange="if (this.value) window.location.href=this.value">
						<option value="province_products.php?prov_id=<?php // echo $getProvId; ?>&sort=relevance" id="relevance"
							<?php // if (isset($_GET['prov_id']) && isset($_GET['sort']) && $_GET['sort'] == 'relevance') { echo "selected"; } ?>
						>Relevance</option>
						<option value="province_products.php?prov_id=<?php // echo $getProvId; ?>&sort=popularity" id="popularity"
							<?php // if (isset($_GET['prov_id']) && isset($_GET['sort']) && $_GET['sort'] == 'popularity') { echo "selected"; } ?>
						>Popularity</option>
						<option value="province_products.php?prov_id=<?php // echo $getProvId; ?>&sort=low_price" id="low_price"
							<?php // if (isset($_GET['prov_id']) && isset($_GET['sort']) && $_GET['sort'] == 'low_price') { echo "selected"; } ?>
						>Price: Low - High</option>
						<option value="province_products.php?prov_id=<?php // echo $getProvId; ?>&sort=high_price" id="high_price" 
							<?php // if (isset($_GET['prov_id']) && isset($_GET['sort']) && $_GET['sort'] == 'high_price') { echo "selected"; } ?>
			...
			<div class="row product-row">
				...
				$query = "SELECT products.*, province.prov_name FROM tbl_products products, tbl_province province WHERE products.prov_id = province.prov_id AND products.prov_id = '$getProvId'";
