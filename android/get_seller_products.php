...
if (isset($securityCode) && !empty($securityCode) && isset($sellerId) && !empty($sellerId) ) {
	...
	$query = "SELECT products.*, province.prov_name, categories.cat_name FROM tbl_products products, tbl_province province, tbl_categories categories WHERE products.prov_id = province.prov_id AND products.cat_id = categories.cat_id AND products.seller_id = '$sellerId'";
	...
	while ($row = mysqli_fetch_assoc($search)) {
		...
		$status = 1;
		$message = "Displaying " . $rowsCount . " seller products.";
