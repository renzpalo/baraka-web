...
if (isset($securityCode) && !empty($securityCode) && isset($productId) && !empty($productId) && isset($userId) && !empty($userId)) {
	...
	$information = "Failed to delete product from wishlist.";
	...
	if ($doesNotExist) {
		...
		$query2 = "DELETE FROM tbl_wishlist WHERE user_id = '$user_id' AND prod_id = '$prod_id'";

		$deleteWishlist = mysqli_query($connect, $query2);
		...
		if ($deleteWishlist) {
			...
			$information = "Successfully deleted product from wishlist.";
		}
	}
