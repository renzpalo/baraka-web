...
$provId = validateFormData($_POST['prov_id']);
...
if (isset($securityCode) && !empty($securityCode) && isset($provId) && !empty($provId) ) {
	if (!$connect) {
	...
	$query = "SELECT products.*, province.prov_name, categories.cat_name FROM tbl_products products, tbl_province province, tbl_categories categories WHERE products.prov_id = province.prov_id AND products.cat_id = categories.cat_id AND products.prov_id = '$provId'";
